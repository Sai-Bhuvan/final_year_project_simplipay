...
async def compare_face(data: CompareFaceRequest, req: Request):
    try:
        db = req.app.mongodb_db
        users = db["merchants"]
        user = await users.find_one({"phoneNo": data.from_})
        if not user or "image" not in user:
            raise HTTPException(status_code=404, detail="User or image not found")

        img1_np = base64_to_image(data.image)
        ...
        return {"message": message, "statusCode": status_code}
